...
.	Make a copy of your current project and call it  `ActionBankAccount`.
...
----
+
.	Declare a class `ActionListBankAgency` in the `application.actionlist` package,
...
.. to create an instance of `ActionListBankAgency`,
.. launch `execute()` on the instance of `ActionListBankAgency`.
+
[WARNING]
====
Is it working ?
